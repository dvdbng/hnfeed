...
class Readability
{
	...
	*/
	public function postProcessContent($articleContent) {
        ...
        $this->convertRelativeURLs($articleContent);
	}
	...
	**/
    ...
    public function convertRelativeURLs($content){
        $html = $this->dom;

        foreach(array("a"=>"href","img"=>"src") as $elm=>$attr){
    		$elms = $content->getElementsByTagName($elm);
            for ($i = 0; $i<$elms->length; $i++){
                $elm = $elms->item($i);
                if($elm->hasAttribute($attr)){
                    $elm->setAttribute($attr,$this->rel2abs($elm->getAttribute($attr)));
                }
            }
        }
    }
